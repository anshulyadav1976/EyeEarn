...
const validCoordinate = (p: RoutePoint) =>
...
export function createRun(
  ...
  if (!runnerName.trim() || runnerName.length > 80)
    throw new Error("Runner name is required");
  if (!zoneIds.length || zoneIds.length > 20)
    throw new Error("Choose at least one bounty zone");
  return {
...
export function appendRoutePoints(
  run: Run,
  points: RoutePoint[],
): { accepted: RoutePoint[]; rejected: number } {
  ...
  for (const point of points.slice(0, 100)) {
    ...
    if (previous) {
      const seconds = Math.max(
        1,
        (Date.parse(point.recordedAt) - Date.parse(previous.recordedAt)) / 1000,
      );
      if (distanceMetres(previous, point) > 300 * seconds) continue;
    }
    accepted.push({
      ...point,
      accuracy:
        point.accuracy == null
          ? null
          : Math.max(0, Math.min(500, point.accuracy)),
    });
  }
  run.routePoints.push(...accepted);
  return { accepted, rejected: points.length - accepted.length };
}
...
export function addObservation(run: Run, observation: Observation) {
  ...
  if (
    ...
    observation.description.length > 500
  )
    throw new Error("Invalid observation");
  if (observation.privacyState === "blocked")
    throw new Error("Blocked evidence cannot be retained");
  run.observations.push({
    ...observation,
    description: observation.description.slice(0, 500),
  });
  return observation;
}
...
export function completeZone(
  ...
  if (run.status !== "live" || !run.zoneIds.includes(zone.id))
    throw new Error("Zone is not part of this live run");
  if (run.completions.some((c) => c.zoneId === zone.id))
    throw new Error("Zone already completed");
  const completion = {
    zoneId: zone.id,
    accepted,
    completedAt: now,
    rewardMinor: accepted ? zone.rewardMinor : 0,
  };
  run.completions.push(completion);
  run.earnedMinor += completion.rewardMinor;
  return completion;
}
...
export function handoffRun(run: Run, now = new Date().toISOString()) {
  if (run.status !== "finished")
    throw new Error("Finish the run before handoff");
